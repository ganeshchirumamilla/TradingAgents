...
- **Interactive Brokers (IBKR) integration**, opt-in via `ibkr_enabled` /
  `TRADINGAGENTS_IBKR_ENABLED` and the new `tradingagents[ibkr]` extra
  (`ib_async`): account/position context injected into the Trader and
  Portfolio Manager prompts, IBKR as a `core_stock_apis` data vendor for
  plain US stocks, and optional order execution (`ibkr_auto_execute`) after
  the Portfolio Manager's decision. Live (non-paper) order placement requires
  the separate `TRADINGAGENTS_IBKR_CONFIRM_LIVE` env var as a deliberate
  second confirmation. See README "Interactive Brokers (IBKR) Integration".
