#!/usr/bin/env python3
"""
Run Swing Trading Strategy backtest for past 1 year.
Uses daily and hourly data from PostgreSQL.
Initial capital: $10,000
Re-invests all profits.

Philosophy: Professional swing trading with conservative risk management
- Wait for clear trends (SMA alignment)
...
def main():
    ...
    strategy = SwingTradingStrategy(
        ...
        max_positions=3         # Max 3 concurrent positions (quality over quantity)
    )
    ...
    if report and not report['trades'].empty:
        output_file = f"swing_trading_backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        report['trades'].to_csv(output_file, index=False)
        logger.info(f"Trades exported to: {output_file}")

        # Export equity curve
        equity_file = f"swing_trading_equity_curve_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        pd.DataFrame(strategy.equity_curve).to_csv(equity_file, index=False)
